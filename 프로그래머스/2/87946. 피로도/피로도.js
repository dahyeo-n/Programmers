function solution(k, dungeons) {
  let answer = 0;
  const visited = Array.from({ length: dungeons.length }, () => false); // 방문했는지 확인하는 배열

  // 완전탐색을 위한 DFS (남은 피로도, 진행단계)
  function DFS(hp, L) {
    for (let i = 0; i < dungeons.length; i++) { // 던전의 수만큼 반복
    ...
    answer = Math.max(answer, L); // 가장 많이 들어간 진행 단계를 answer에 넣음
  }
